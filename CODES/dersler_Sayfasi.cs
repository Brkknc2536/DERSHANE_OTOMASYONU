...
namespace Birebir_Etut_Programi
{
    ...
    {  //oluşturduğumuz sql bağlantı sınıfından nesne türetiyoruz
        ...
        //derslistele adında fonksiyon oluşturduk tabloda daha güzel gözükmesi için as sql kelimesi ile columnlara isim verdik ve datagridwiewe aktardık.
        void DersListele()
        {
        ...
        {//yaptığımız ders listeleme fonksiyonunu çağırdık
            DersListele();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                if (TxtDers.Text == "")//boş alanları kontrol ettirdik eğer varsa doldurması için mesaj yazdık.
                {
                ...
                {
                    try
                    {//sql ekleme komutu ile dersi ekleme  işlemi yaptık
                        ...
                        //ardından yeni oluşan dersi göstermesi için dersListele fonksiyonumuzu çağırdık
                        DersListele();
                        TxtDers.Text = "";
                    }
                    catch (Exception)//herhangi bir hata durumunda mesaj vermesi için hazırladık.
                    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (secim == DialogResult.Yes)//ders silmek istiyor mu sorusu sorduk eğer evet ise içeri girip işlem yapacak
                {
                    try
                    {//sql ekleme komutu ile dersi silme  işlemi yaptık
                        SqlCommand komut = new SqlCommand("Delete From TBL_DERSLER where DERSID='"+ textBox2.Text+"'", bgl.baglanti());
                      
                        komut.ExecuteNonQuery();
                        bgl.baglanti().Close();
                        DersListele();
                        //ardından yeni oluşan ders listesini göstermesi için dersListele fonksiyonumuzu çağırdık
                        TxtSilinecek.Text = "";
